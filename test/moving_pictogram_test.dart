...
void main() {
  ...
  group('MovingPictogram', () {
    testWidgets('animates when it has a motion', (WidgetTester tester) async {
      await tester.pumpWidget(host(const MovingPictogram(
        motion: VocabMotion.travel,
        size: 40,
        child: SizedBox(width: 40, height: 40, key: ValueKey<String>('art')),
      )));

      final Offset start = tester.getCenter(find.byKey(const ValueKey<String>('art')));
      await tester.pump(const Duration(milliseconds: 650));
      final Offset moved = tester.getCenter(find.byKey(const ValueKey<String>('art')));
      expect(moved.dx, isNot(closeTo(start.dx, 0.5)),
    ...
    testWidgets('a still motion adds no animation at all',
      ...
      final Offset start = tester.getCenter(find.byKey(const ValueKey<String>('art')));
      await tester.pump(const Duration(milliseconds: 650));
      expect(tester.getCenter(find.byKey(const ValueKey<String>('art'))), start);
    });
    ...
    testWidgets('switching the word switches the motion',
      ...
      await tester.pump(const Duration(milliseconds: 650));
      expect(tester.getCenter(find.byKey(const ValueKey<String>('art'))),
          settled);
    });
  });

  group('through VocabIcon', () {
    testWidgets('a verb gets a moving pictogram, a noun a still drawing',
        (WidgetTester tester) async {
      debugResetVocabIcons();
      await loadVocabIconIndex(rootBundle);

      await tester.pumpWidget(host(VocabIcon(word: wordWithId('x10018'))));
      expect(find.byType(MovingPictogram), findsOneWidget,
          reason: 'gehen is a verb with a travel pictogram');
